...
Cardio Pulse PRO is a heart rate (HR) and heart rate variability (HRV) monitoring device designed for stress and recovery analysis, developed at Metropolia university of Applied Sciences. The device is built with a Raspberry Pi Pico W and a Crowtail PPG pulse sensor. It detects peaks from a live signal, calculates HR and HRV metrics locally, and sends interval data to Kubios Cloud for advanced analysis. The device includes an OLED display and rotary encoder for simple navigation, and shows both real-time and historical measurements. 
